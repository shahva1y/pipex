...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/01/15 17:00:57 by krulindi          #+#    #+#             */
/*   Updated: 2022/01/15 17:00:59 by krulindi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_get_command_path(char *command, char **dirs)
{
	...
	if (!access(command, 0))
		return (command);
	command = (char *) ft_strjoin("/", command);
	if (!command)
		ft_error("Ошибка выделения памяти!");
	while (dirs[i] != 0)
	{
		command_path = (char *)ft_strjoin(dirs[i], command);
		if (!command_path)
			ft_error("Ошибка выделения памяти!");
